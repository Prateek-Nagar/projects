...
public class PasswordController {
    ...
    public ModelAndView getResetPasswordPage(@RequestParam String token) {
        return new ModelAndView("auth/resetPassword").addObject("token", token);
    }


    @RequestMapping(value = "/forget-password-process", method = RequestMethod.POST)
    public RedirectView sendPasswordRestMail(@RequestParam String email, RedirectAttributes redirectAttributes) {

        RedirectView redirectView=new RedirectView("auth/login");
        RedirectView redirectView1=new RedirectView("auth/forgetPassword");
        ...
        //redirectAttributes.addFlashAttribute("forgetPassword", "Success");
    ...
    public String sendPasswordRestMail(@RequestParam String email, RedirectAttributes redirectAttributes) {
        ...
        String emailBody = "http://localhost:8080/reset-password?token=";
        String resetUrl = signUpService.saveTokenAndGenerateResetUrl(user, emailBody);
        emailService.sendEmailToSingleRecipient(email, resetUrl);
        redirectAttributes.addFlashAttribute("forgetPassword", "Success");
        return "auth/login";
    }
*/
